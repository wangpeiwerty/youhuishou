...
public class RadioGroupDialog extends BaseAlertDialog<RadioGroupDialog> {
    /** title underline */
    ...
    public RadioGroupDialog(Context context,String[] titles,boolean[] checked) {
        super(context);
        this.titles=titles;
    ...
    public View onCreateView() {
        ...
        /** content */
        radiogroup = new LinearLayout(mContext);
        radiogroup.setOrientation(LinearLayout.VERTICAL);
        radiogroup.setPadding(40,5,5,5);
        for (int i=0;i<titles.length;i++){
            AppCompatCheckBox appCompatRadioButton = new AppCompatCheckBox(mContext);
            if(i==0){
                appCompatRadioButton.setEnabled(false);
            }
            appCompatRadioButton.setText(titles[i]);
            appCompatRadioButton.setChecked(checked[i]);
            radiogroup.addView(appCompatRadioButton);
        }
        radiogroup.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT));
        mLlContainer.addView(radiogroup);
    ...
    /** set style(设置style) */
    public RadioGroupDialog style(int style) {
        this.mStyle = style;
        return this;
    }

    /** set title underline color(设置标题下划线颜色) */
    public RadioGroupDialog titleLineColor(int titleLineColor) {
        this.mTitleLineColor = titleLineColor;
        return this;
    }

    /** set title underline height(设置标题下划线高度) */
    public RadioGroupDialog titleLineHeight(float titleLineHeight_DP) {
        this.mTitleLineHeight = titleLineHeight_DP;
        return this;
    }

    /** set divider color between btns(设置btn分割线的颜色) */
    public RadioGroupDialog dividerColor(int dividerColor) {
